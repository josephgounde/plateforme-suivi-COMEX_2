    ...
    public async Task<OtpChallengeResponse> Handle(LoginCommand command, CancellationToken cancellationToken)
    {
        ...
        if (utilisateur is null)
        {
            ...
            throw new AuthException(403, AuthErrorResponseCode.NO_PROFILE_MAPPED,
                "Aucun profil GSA-COMEX associé à ce compte. Contactez l'administrateur DSIRI.");
        }
        ...
        var resultatEnvoi = await sender.EnvoyerAsync("EMAIL", utilisateur.Email, "Code de vérification GSA-COMEX",
            $"Votre code de vérification GSA-COMEX est : {code}. Il est valable {OtpSettings.ValiditeSecondes / 60} minutes.",
            cancellationToken);
