...
// Duplique PDOE.Workflow.API.Common.WorkflowEngine (modules indépendants, pas de ProjectReference).
// Le routage suit WorkflowEtapes.Ordre/Actif en direct, pas une table de transition figée.
internal static class WorkflowEngine
{
    ...
    public static async Task AvancerVersEtapeSuivante(PdoeDbContext db, Dossier dossier, CancellationToken cancellationToken)
    {
        ...
        // ETAPE_1 et ETAPE_7 sont seedées en GENERIQUE mais restent suivies via StatutDossier ;
        // EtapeGeneriqueCode ne sert que pour les étapes hors des 7 codes historiques.
        if (EtapeVersStatutEntree.TryGetValue(suivante.Code, out var statutCible))
        {
            dossier.StatutElectronique = statutCible.ToString();
        }
        else
        {
            dossier.EtapeGeneriqueCode = suivante.Code;
